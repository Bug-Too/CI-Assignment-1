...
class MLP():
    ...
    def __init__(self, layers, bias, learningRate, momentumRate, maxEpoch, epsilon, filePath, startLine, validationNumber, validationRange) -> None:
        file = readFile(filePath, startLine, validationNumber, validationRange)
        self.data = file.data
    ...
    def findError(self, positionOfData, isValidate):
        if isValidate:
            ...
            for i in range(len(self.nodeValue[-1])):
                temp.append(self.validationDesireOutput[positionOfData][i] - self.nodeValue[-1][i][0])
            return temp
        else:
            temp = []
            for i in range(len(self.nodeValue[-1])):
    ...
    def findGrad(self):
        ...
        for outputPosition in range(len(self.grad[-1])):
            self.grad[-1][outputPosition] = [self.listOfError[-1][outputPosition] *
                                             mathOperation.diffActivationFunc(self.inputNode[-1][outputPosition])[0]]
        # hidden
        i = len(self.grad) - 2
        while (i >= 0):
            j = 0
            while (j < len(self.grad[i])):
                self.grad[i][j] = [
                    mathOperation.diffActivationFunc(self.inputNode[i][j])[0] * self.sumOfProductOfWeightAndGrad(i, j)]
                j = j + 1
            i = i - 1
